...
CLI 처리 외에, 브라우저에서 파일을 올리고 결과를 화면에서 바로 확인하는 **로컬 웹 UI**를 제공한다.
로컬 전용으로만 동작하며(`127.0.0.1`, 외부 업로드 없음 — 10.5 준수), 화면 구성은 네이버 파파고 형태의 좌/우 2단을 따른다.

#### 9.3.1 전체 디자인 가이드

- 왼쪽 영역은 첨부파일 업로드, 오른쪽 영역은 추출 텍스트 결과물 출력 영역으로 정의한다.

#### 9.3.2 헤더

- 헤더 중앙에 "로컬 STT" 타이틀을 배치한다.

#### 9.3.3 첨부파일 업로드 영역 (왼쪽)

- 정사각형에 가까운 드래그앤드롭 및 파일 선택 영역을 그린다. 영역의 border는 자연스럽게(둥근 모서리·점선 등) 처리한다.
- 첨부 영역 아래 "추출하기(green) / 삭제하기(gray)" 버튼을 50%씩 배치한다. 드래그앤드롭 영역이 절대적으로 크고, 버튼은 하단 약 10% 높이를 차지하게 한다.
- 첨부파일 영역 하단에 업로드한 파일의 음성 길이를 표시한다. (추출 전, 브라우저에서 즉시 계산)
- 첨부파일 영역 바로 밑에 화자 구분 체크박스를 배치하고, 옆 괄호로 시간이 오래 걸린다는 안내 문구를 표시한다.

#### 9.3.4 결과물 영역 (오른쪽)

- 텍스트 폰트는 11px로 출력한다.
- 30줄가량의 텍스트를 한 번에 보여주고, 초과할 경우 스크롤바를 생성한다.
- 결과물 영역 우측 하단에 전체 내용을 복사하는 이모티콘과 기능을 둔다. 출력 텍스트와 겹치지 않도록 별도 영역을 부여한다.
- 결과물 영역 하단에 "총 단어 수 : n 개 / 총 문자 수 : m 개"를 표시한다.
- 결과물 영역 하단 우측에 결과를 `.txt` 파일로 내려받는 다운로드 버튼을 둔다. 이 버튼은 첨부파일 영역의 버튼과 별개로 구현한다.

#### 9.3.5 진행/상태 처리

- 음성파일 업로드 후 추출 진행 중에는 업로드 영역을 비활성화하여 삭제/추출 버튼의 중복 클릭을 막는다.
- 음성 추출 진행 상황을 화면 중앙의 progress bar로 보여주고, 전체 화면을 어둡게 처리하여 progress bar를 부각한다.
- 화면 우측 상단에 "작업 취소하기(red)" 버튼을 두고, 화면에 스크롤이 생겨도 어떤 위치에서나 항상 고정 위치에 노출되게 처리한다.
...
- 브라우저 업로드 기반 로컬 웹 UI 적용 (9.3 설계 기준)
- 좌/우 2단 레이아웃(업로드 / 결과)
- 음성 길이 표시, 화자 구분 옵션 토글
- 진행률 표시(중앙 progress bar + 화면 딤), 작업 취소 버튼
- 결과 복사, 단어/문자 수 표시, `.txt` 다운로드
